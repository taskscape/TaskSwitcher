...
namespace ManagedWinapi
{
    ...
    public class CodepointRange
    {
        ...
        /// </summary>
        private char FirstCodepoint => ranges[0];

        /// <summary>
        /// The last (highest) supported codepoint.
        /// </summary>
        private char LastCodepoint => (char)(ranges[ranges.Length - 1] - 1);

        /// <summary>
        /// Tests whether a specific codepoint is supported by this font.
        /// </summary>
        private bool IsSupported(char codepoint)
        {
            return ranges.TakeWhile(c => c <= codepoint).Aggregate(false, (current, c) => !current);
        }
